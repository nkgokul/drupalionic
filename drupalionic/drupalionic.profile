 ...
 */
function drupalionic_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function drupalionic_install_tasks() {
  $tasks = array();

  $tasks['drupalionic_setup_blocks'] = array(
    'display_name' => st('Setup Blocks'),
    'display' => FALSE,
  );

  // Run this as the last task!
  $tasks['drupalionic_setup_rebuild_permissions'] = array(
 ...
 */
function drupalionic_setup_blocks() {
 ...
 * the installation.
 */
function drupalionic_setup_rebuild_permissions() {
  node_access_rebuild();
}
